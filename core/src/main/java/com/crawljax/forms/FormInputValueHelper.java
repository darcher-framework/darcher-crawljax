...
public final class FormInputValueHelper {
     ...
     */
    public FormInput getFormInputWithIndexValue(EmbeddedBrowser browser, Document dom, Node element,
                                                int indexValue) {
        return getFormInput(browser, dom, element, indexValue);
    }

    private FormInput getFormInput(EmbeddedBrowser browser, Document dom, Node element, int indexValue) {

        FormInput matchedInput = formInputMatchingNode(dom, element);
     ...
     * <p>
     * TODO troublor modify: add dom parameter
     *
     * @param element
     * @param dom
     * @return
     */
    private FormInput formInputMatchingNode(Document dom, Node element) {
        NamedNodeMap attributes = element.getAttributes();
        Identification id;
        ...
        // TODO troublor modify starts: check if the element matches any form input by try to find the element using
        //  xpath
        for (Map.Entry<Identification, FormInput> entry :
                this.formInputs.entrySet()) {
            if (entry.getKey().getHow() != Identification.How.xpath) {
                // if the identification is not by xpath, skip it
                continue;
            }
            try {
                Node found = DomUtils.getElementByXpath(dom, entry.getKey().getValue());
                if (found != null && found.equals(element)) {
